...
import { IDisposable, IExtensionContext, IDisposableRegistry } from './platform/common/utils/typesAndConsts';
import { concatMultilineString, formatStreamText, isJupyterNotebook } from './platform/common/utils/utils';
...
import {
    ...
    CellExecutionCreator  // Originally it was elsewhere
} from './kernelExecutionHelpers';
import { Identifiers, WIDGET_MIMETYPE } from './platform/common/utils/typesAndConsts';
    ...
    start(startTime?: number): void {
        ...
        if (this._endCallback) {
            try {
                ...
                // traceInfo( `End cell ${this.cell.index} execution after ${  ((endTime || 0) - (this._startTime || 0)) / 1000 }s, completed @ ${endTime}, started @ ${this._startTime}` );
            } finally {
                this._endCallback();
...
export class CellExecutionMessageHandler implements IDisposable {
    ...
    private onKernelIOPubMessage(_: unknown, msg: KernelMessage.IIOPubMessage<KernelMessage.IOPubMessageType>) {
        ...
        try {
            ...
            if (!this.completedExecution && !this.cell.document.isClosed) {
                ...
                this._onErrorHandlingIOPubMessage.fire(ex as { error: Error; msg: IIOPubMessage<IOPubMessageType>; });
            }
        }
    }
        ...
        if ('msg_id' in data.state && typeof data.state.msg_id === 'string') {
            ...
            data.state.children.forEach((item) => {
                ...
                if (!item.startsWith(IPY_MODEL_PREFIX)) {
                    // return traceWarning( `Came across a comm update message a child that start start with ${IPY_MODEL_PREFIX}`, item);
                }
    ...
    private addToCellData(
        ...
        // traceCellMessage( this.cell, () => `Update output with mimes ${cellOutput.items.map((item) => item.mime).toString()}` );
    ...
    private updateJupyterOutputWidgetWithOutput(
        ...
        const newOutputs = cell.outputs.slice().filter((item) => {
            if (clearWidgetOutput) {
                ...
                return true;  // !outputsOwnedByWidgetModel.has(item.id);  // CHANGE: idk what .id is
            } else {
                return true;
            }
        });
        ...
        if (outputToAppend) {
            ...
            // outputsOwnedByWidgetModel.add(outputToAppend.id); // CHANGE: idk what .id is
        }
    ...
    private handleDisplayData(msg: KernelMessage.IDisplayDataMsg) {
        const output = {
            ...
            data: msg.content.data, // CHANGE: Screw TensorBoard  handleTensorBoardDisplayDataOutput(msg.content.data),
            metadata: msg.content.metadata,
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            transient: msg.content.transient as any // NOSONAR
        };
    ...
    private handleUpdateDisplayDataMessage(msg: KernelMessage.IUpdateDisplayDataMsg) {
        ...
        if (outputToBeUpdated.outputItems.length === newOutput.items.length) {
            ...
            if (allAllOutputItemsSame) {
                ...
                // traceVerbose( 'Update display data message received, but no output to update (data is the same)', msg.content );
                return;
            }
        }
        // Possible execution of cell has completed (the task would have been disposed).
        ...
        if (outputMetadataHasChanged) {
            ...
            const newOutputs = outputToBeUpdated.cell.outputs.map((o: NotebookCellOutput) => {  // CHANGE: There was no type hint
