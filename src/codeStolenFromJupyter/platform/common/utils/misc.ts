...
import { InterpreterUri, Resource, NotebookCellScheme } from './typesAndConsts';
...
export function tracing<T>(log: (t: TraceInfo) => void, run: () => T, logBeforeCall?: boolean): T {
    ...
    try {
        ...
        log({ elapsed: timer.elapsedTime, err: (ex as Error) });  // CHANGE: This type assertion wasnt here
        throw ex;
    }
}
